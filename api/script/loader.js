...
local function makal_request(data)
  ...
  if R then return R(data)
  elseif data.Method == "GET" then return { Body = game:HttpGet(data.Url) }
  else error("Unsupported executor") end
end

local I = makal_request({
  Url = "${process.env.BASE_URL}/api/init?userid=" .. P.UserId .. "&username=" .. P.Name,
  Method = "GET"
})
...
local S = makal_request({
  Url = "${process.env.BASE_URL}/api/script/" .. N,
  Method = "POST",
